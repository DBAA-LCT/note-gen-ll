...
export class HarnessSdkJsonRpcServer {
  ...
  constructor(
    ...
    this.disposers.push(ctx.on('approval/request', async (request: ApprovalRequest, next) => {
      const owned = this.sessions.get(String(request.agent.session.id))
      if (owned?.handle.agent !== request.agent) return next()
      const outcome = await this.transport.request('user.approval', {
    ...
    if (userQuestions !== undefined) {
      this.disposers.push(userQuestions.registerProvider({
        ask: async (request: AskUserQuestionRequest): Promise<AskUserQuestionAnswer> => {
          const answer = await this.transport.request('user.questions', {
            sessionId: request.agent === undefined ? undefined : String(request.agent.session.id),
            questions: request.questions,
          })
          if (!isQuestionAnswer(answer)) throw new Error('native client returned an invalid user-question answer')
          return answer
        },
      }))
    }
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    await this.waitForConfiguredAdapter(this.provider)
  ...
  /**
   * The settings-backed adapter registers just after the loader exposes this
   * stdio server. A native client initializes immediately after spawning, so
   * give that startup publication a bounded window instead of racing it.
   */
  private async waitForConfiguredAdapter(provider: string): Promise<void> {
    if (provider === 'deepseek-official' || this.hasAdapterFor(provider)) return
    const deadline = Date.now() + 5_000
    while (Date.now() < deadline) {
      await new Promise(resolveWait => setTimeout(resolveWait, 25))
      if (this.hasAdapterFor(provider)) return
    }
  }
}
...
function isQuestionAnswer(value: unknown): value is AskUserQuestionAnswer {
