...
var HarnessSdkJsonRpcServer = class {
	...
	constructor(ctx, transport, options = {}) {
		...
		this.disposers.push(ctx.on("approval/request", async (request, next) => {
			if (this.sessions.get(String(request.agent.session.id))?.handle.agent !== request.agent) return next();
		...
		if (userQuestions !== void 0) this.disposers.push(userQuestions.registerProvider({ ask: async (request) => {
			const answer = await this.transport.request("user.questions", {
				sessionId: request.agent === void 0 ? void 0 : String(request.agent.session.id),
	...
	*/
	async waitForConfiguredAdapter(provider) {
...
function isQuestionAnswer(value) {
	return typeof value === "object" && value !== null && !Array.isArray(value) && typeof value.answers === "object" && value.answers !== null;
}
//#endregion
