...
export class HarnessSdkJsonRpcServer {
    ...
    constructor(ctx, transport, options = {}) {
        ...
        this.disposers.push(ctx.on('approval/request', async (request, next) => {
            ...
            if (owned?.handle.agent !== request.agent)
                return next();
            ...
            this.disposers.push(userQuestions.registerProvider({
                ask: async (request) => {
                    ...
                    if (!isQuestionAnswer(answer))
                        throw new Error('native client returned an invalid user-question answer');
                    return answer;
                },
            }));
        }
    ...
    async waitForConfiguredAdapter(provider) {
        if (provider === 'deepseek-official' || this.hasAdapterFor(provider))
            return;
