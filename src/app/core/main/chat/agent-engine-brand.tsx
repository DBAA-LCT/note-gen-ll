...
import { AGENT_ENGINE_LOGO_DATA } from './agent-engine-logo-data'
...
export function AgentEngineMark({ engine, className }: { engine: AgentEngineId; className?: string }) {
  ...
  return (
    <span className={cn('flex size-6 shrink-0 items-center justify-center overflow-hidden rounded-md', visual.markClassName, className)}>
      {engine === 'native' ? (
        <Sparkles className="size-[62%]" aria-hidden="true" />
      ) : engine === 'claude' ? (
        <svg className="size-[82%]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        <Image
          src={AGENT_ENGINE_LOGO_DATA[engine]}
          alt=""
          width={48}
          height={48}
          unoptimized
