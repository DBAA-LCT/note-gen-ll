...
import { GoalDialog } from '@/features/learning/goal-dialog'
...
const workspaceItems = [
  ...
  { view: 'review' as const, label: '知识复习', icon: Brain },
  { view: 'reports' as const, label: '日报', icon: NotebookPen },
  { view: 'periods' as const, label: '周期报告', icon: ChartNoAxesColumnIncreasing },
]
...
export function LearningSidebar() {
  ...
  const completionPercent = actionableTasks.length ? Math.round(completedTasks / actionableTasks.length * 100) : 0
  ...
  const createManualTask = async () => {
    ...
    try {
      await addManualTask({
        ...
        plannedMinutes: Math.max(5, taskMinutes || 5),
    ...
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
        ...
        <div className="grid grid-cols-2 gap-1 border-b pb-3">
          {workspaceItems.slice(1).map(({ view, label, icon: Icon }) => (
            <Button key={view} size="sm" variant="ghost" className="justify-start px-2 text-xs" onClick={() => void openLearningWorkspace(view)}>
        ...
        <section className="border-b py-3">
          ...
          <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
            ...
            <button type="button" className="hover:text-foreground" onClick={() => void openLearningWorkspace('focus')}>开始专注</button>
          </div>
        </section>
        ...
        <section className="py-3">
          <div className="mb-1 flex h-7 items-center justify-between px-1">
            ...
            <div className="flex items-center gap-0.5">
              {activeFilePath ? <Button size="icon-xs" variant="ghost" title="把当前笔记加入学习任务" onClick={() => openTaskDialog(true)}><Link2 /></Button> : null}
          ...
          {!loading && !actionableTasks.length ? (
            <button type="button" className="flex w-full items-center gap-2 rounded-md px-2 py-3 text-left text-xs text-muted-foreground hover:bg-muted/60 hover:text-foreground" onClick={() => setGoalOpen(true)}>
              <Plus className="size-3.5" />创建目标后生成今日任务
            </button>
          ) : null}
          {actionableTasks.slice(0, 6).map((task) => (
            <div key={task.id} className="group flex min-h-10 items-start gap-2 rounded-md px-2 py-2 hover:bg-muted/60">
              ...
              <button type="button" className="min-w-0 flex-1 text-left" onClick={() => void openLearningWorkspace('today')}>
                ...
                <span className="block truncate text-[11px] text-muted-foreground">{task.goalTitle ? `${task.goalTitle} · ` : ''}{task.plannedMinutes} 分钟</span>
              </button>
            </div>
          ))}
      ...
      <Dialog open={taskOpen} onOpenChange={setTaskOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>添加学习任务</DialogTitle><DialogDescription>任务会加入今天的学习清单，也可以关联当前 NoteGen 笔记。</DialogDescription></DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2"><Label htmlFor="learning-task-minutes">预计分钟数</Label><Input id="learning-task-minutes" type="number" min={5} max={720} value={taskMinutes} onChange={(event) => setTaskMinutes(Number(event.target.value))} /></div>
            {activeFilePath ? (
