...
import { AgentEngineMark } from '@/app/core/main/chat/agent-engine-brand'
  ...
  return <Card>
    ...
    <CardContent className="flex flex-col gap-3">
      <div className="flex items-center justify-between rounded-lg border p-3">
        <div className="flex items-center gap-3"><AgentEngineMark engine="native" /><div><div className="font-medium">NoteGoal 内置 Agent</div><div className="text-sm text-muted-foreground">无需外部 CLI，保留为故障回退</div></div></div>
      ...
      {AGENT_ENGINE_CATALOG.map(item => {
        ...
        return <div key={item.id} className="rounded-lg border p-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="flex items-center gap-3"><AgentEngineMark engine={item.id} /><div><div className="font-medium">{item.name}</div><div className="text-sm text-muted-foreground">{item.description}</div></div></div>
