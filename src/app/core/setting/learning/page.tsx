  ...
  const save = async () => {
    ...
    try {
      await updateSettings({
        ...
        reportDirectory: draft.reportDirectory.trim() || '规划报告/日报',
      })
      toast.success('规划设置已保存')
    } catch (error) {
      toast.error('保存规划设置失败', { description: error instanceof Error ? error.message : String(error) })
    }
  }

  return (
    <SettingType id="learning" title="规划" icon={<Flag />} desc="管理目标、每日任务、完成规则与规划报告。">
      <SettingSection title="每日规划" desc="自动任务会按每日可投入时间和目标权重分配，不会覆盖手工任务。">
        <div className="grid gap-4 rounded-lg border p-4 sm:grid-cols-2">
          <div className="space-y-2"><Label htmlFor="learning-daily-budget">每日可投入时间（分钟）</Label><Input id="learning-daily-budget" type="number" min={15} max={720} value={draft.dailyStudyMinutes} onChange={event => setDraft({ ...draft, dailyStudyMinutes: Number(event.target.value) })} /></div>
      ...
      <SettingSection title="目标与规划报告">
        <div className="space-y-4 rounded-lg border p-4">
          <div className="space-y-2"><Label htmlFor="learning-report-directory">规划报告目录</Label><Input id="learning-report-directory" value={draft.reportDirectory} onChange={event => setDraft({ ...draft, reportDirectory: event.target.value })} /><p className="text-xs text-muted-foreground">报告会作为只读 Markdown 文档显示，修改请回到规划表单。</p></div>
      ...
      <div className="flex justify-end"><Button onClick={() => void save()}>保存规划设置</Button></div>
      <SettingSection title="外部服务" desc="把其他工具的进度汇总到规划中。">
        <MaimemoSettingsCard />
      </SettingSection>
    </SettingType>
  )
